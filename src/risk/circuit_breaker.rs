...
impl CircuitBreaker {
    ...
    pub fn record_trade(&mut self, pnl: f64, new_balance: f64) {
        ...
        if pnl < 0.0 {
            ...
            if self.consecutive_losses > 0 { info!("✅ Profit recorded - consecutive loss streak broken"); }
    ...
    pub fn status(&self) -> CircuitBreakerStatus {
        CircuitBreakerStatus {
            ...
            cooldown_remaining:  self.trip_time.map(|t| {
                ...
                if elapsed < self.cooldown_duration { self.cooldown_duration - elapsed } else { Duration::ZERO }
            }),
        }
    }
}
...
mod tests {
    ...
    fn test_record_trade_noop_when_tripped() {
        let config = test_config();
        let mut breaker = CircuitBreaker::with_balance(&config, 10000.0);
        for _ in 0..5 { breaker.record_trade(-100.0, 9900.0); }
        assert!(breaker.is_tripped);
    ...
    fn test_profit_resets_streak() {
        let config = test_config();
        let mut breaker = CircuitBreaker::with_balance(&config, 10000.0);
        for _ in 0..3 { breaker.record_trade(-50.0, 9900.0); }
