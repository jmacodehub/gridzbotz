...
//! February 27, 2026 - V1.1: Fix grid_efficiency double-scale (×100 twice)
//!                     grid_efficiency is stored as a ratio (0.0–1.0).
//!                     Threshold comparisons were correct; only the log
//!                     format strings multiplied by 100 again, producing
//!                     "Efficiency 4857.1%" and always triggering the
//!                     high-efficiency size-up path on every pass.
//! ═════════════════════════════════════════════════════════════════════════
...
#[allow(dead_code)] // semantic label; else-branch catches > 8%
...
// These are RATIOS (0.0–1.0), matching how grid_efficiency is stored.
const HIGH_EFFICIENCY_THRESHOLD: f64 = 0.70;  // > 70% = scale up
const LOW_EFFICIENCY_THRESHOLD: f64  = 0.30;  // < 30% = scale down
...
/// # Units contract
///
/// `grid_efficiency` in [`EnhancedMetrics`] is stored as a **ratio** in
/// the range `0.0 – 1.0` (e.g. `0.486` = 48.6%).  All threshold
/// comparisons below use the same ratio scale.  When formatting for
/// human-readable output multiply by `100.0` inside the format string.
#[derive(Debug, Clone)]
...
impl AdaptiveOptimizer {
    ...
    /// Returns `true` if spacing changed by more than 5%.
    pub fn update_spacing(&mut self, metrics: &EnhancedMetrics) -> bool {
        ...
        if change_pct > 5.0 {
            ...
            // drawdown is stored as a percentage already (e.g. 2.0 = 2%)
            let drawdown = metrics.max_drawdown;
    ...
    ///
    /// `metrics.grid_efficiency` is a **ratio** (0.0 – 1.0).
    /// Threshold constants are defined in the same ratio scale.
    pub fn calculate_optimal_position_size(&self, metrics: &EnhancedMetrics) -> f64 {
        // efficiency is a ratio, e.g. 0.486 = 48.6%
    ...
    /// Returns `true` if size changed by more than 10%.
    pub fn update_position_size(&mut self, metrics: &EnhancedMetrics) -> bool {
        ...
        if change_pct > 10.0 {
            ...
            // V1.1 fix: efficiency is a ratio (0.0–1.0); multiply by 100
            // here in the format string — NOT before passing into this fn.
            let efficiency = metrics.grid_efficiency;
            self.last_reason = format!(
...
mod tests {
    ...
    fn test_optimizer_creation() {
        let opt = AdaptiveOptimizer::new(0.15, 0.1);
        assert_eq!(opt.base_spacing_percent,   0.15);
        assert_eq!(opt.base_position_size,     0.1);
        assert_eq!(opt.current_spacing_percent, 0.15);
    ...
    fn test_spacing_tightens_on_low_drawdown() {
        ...
        m.max_drawdown = 1.0; // low drawdown
    ...
    fn test_spacing_widens_on_high_drawdown() {
        let opt = AdaptiveOptimizer::new(0.15, 0.1);
        let mut m = EnhancedMetrics::new();
        m.max_drawdown = 10.0; // high drawdown
        assert!(opt.calculate_optimal_spacing(&m) > opt.base_spacing_percent);
    }
    ...
    fn test_position_size_scales_with_efficiency() {
        ...
        m.grid_efficiency = 0.80; // 80% — ratio, high
        let high = opt.calculate_optimal_position_size(&m);

        m.grid_efficiency = 0.20; // 20% — ratio, low
    ...
    fn test_limits_enforced() {
        let opt = AdaptiveOptimizer::new(0.01, 0.01);
        let mut m = EnhancedMetrics::new();
        m.max_drawdown = 50.0;
    ...
    /// V1.1 regression: efficiency=0.486 must NOT trigger the high-efficiency
    /// path (threshold is 0.70).  Before the fix, the hallucinated 4857% value
    /// always exceeded 0.70 and bumped position size on every pass.
    #[test]
    fn test_no_size_up_at_moderate_efficiency() {
        let opt = AdaptiveOptimizer::new(0.15, 0.1);
        let mut m = EnhancedMetrics::new();
        m.grid_efficiency = 0.486; // 48.6% — moderate, should be SIZE_NORMAL

        let size = opt.calculate_optimal_position_size(&m);
        // With normal multiplier (1.0) and no streak bonus, size == base
        assert!(
            (size - opt.base_position_size).abs() < 1e-9,
            "Expected base size {}, got {}",
            opt.base_position_size, size
        );
    }
}
