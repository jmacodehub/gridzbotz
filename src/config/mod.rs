//! ═══════════════════════════════════════════════════════════════════════════
//! 🎛️ UNIFIED CONFIGURATION SYSTEM V3.5 - PROJECT FLASH
//! 
...
//! 
//! October 17, 2025 - MASTER V3.5 PRODUCTION READY 🚀
//! ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// MAIN CONFIG IMPLEMENTATION - V3.5 PRODUCTION GRADE! 🚀
// ═══════════════════════════════════════════════════════════════════════════
...
impl Config {
    ...
    pub fn display_summary(&self) {
        ...
        println!("\n{}", border);
        println!("  🤖 PROJECT FLASH V3.5 - CONFIGURATION");
        println!("{}\n", border);
...
impl ConfigPresets {
    /// Conservative preset (low risk, wide spacing)
    pub fn conservative() -> Result<Config> {
        ConfigBuilder::new()
    ...
    /// Balanced preset (moderate risk, medium spacing)
    pub fn balanced() -> Result<Config> {
        ConfigBuilder::new()
            .environment("production")
            .grid_spacing(0.2)
            .grid_levels(25)
    ...
    /// Aggressive preset (high frequency, tight spacing)
    pub fn aggressive() -> Result<Config> {
        ConfigBuilder::new()
            .environment("production")
            .grid_spacing(0.1)
            .grid_levels(50)
    ...
    /// Testing preset (disabled safety features)
    pub fn testing() -> Result<Config> {
        ConfigBuilder::new()
            .environment("testing")
            .grid_spacing(0.15)
            .grid_levels(35)
...
mod tests {
    ...
    fn test_config_builder() {
        ...
        assert!(!config.trading.enable_regime_gate);
        assert_eq!(config.trading.grid_spacing_percent, 0.15);
    }
    
    #[test]
    fn test_duration_calculation() {
        let mut pt_config = PaperTradingConfig::default();
        
        // Test hours
        pt_config.test_duration_hours = Some(2);
        assert_eq!(pt_config.duration_seconds(), 7200);
        
        // Test minutes (overrides hours)
        pt_config.test_duration_minutes = Some(30);
        assert_eq!(pt_config.duration_seconds(), 1800);
        
        // Test seconds (overrides all)
        pt_config.test_duration_seconds = Some(300);
        assert_eq!(pt_config.duration_seconds(), 300);
    }
    
    #[test]
    fn test_presets() {
        assert!(ConfigPresets::conservative().is_ok());
        assert!(ConfigPresets::balanced().is_ok());
        assert!(ConfigPresets::aggressive().is_ok());
        assert!(ConfigPresets::testing().is_ok());
    }
}

// Re-exports for convenience
pub use Config as MainConfig;
