#!/bin/bash
#═══════════════════════════════════════════════════════════════════════════
# 🤖 CONFIG BATTLE ROYALE - AI OPTIMIZATION SYSTEM
#═══════════════════════════════════════════════════════════════════════════
#
...
# Default configs to battle
DEFAULT_CONFIGS=(
    "config/optimized/conservative_v4.toml"
    "config/optimized/balanced_v4.toml"
    "config/optimized/multi_strategy_v4_conservative_ai.toml"
)
...
for i in "${!CONFIGS[@]}"; do
    ...
    echo -e "${BOLD}${YELLOW}[$((i+1))]${RESET} ${GREEN}$NICKNAME${RESET}"
    echo "    ${CYAN}→${RESET} $CONFIG"
done

echo ""
echo -e "${BOLD}${RED}⚠️  STARTING BATTLE IN 5 SECONDS...${RESET}"
echo ""
sleep 5
...
for i in "${!CONFIGS[@]}"; do
    ...
    echo -e "${CYAN}[Battle $((i+1))]${RESET} Starting ${GREEN}$NICKNAME${RESET}..."
...
while true; do
    ...
    for i in "${!CONFIGS[@]}"; do
        ...
        echo -e "${BOLD}[$((i+1))] $NICKNAME${RESET}"
...
echo -e "${BOLD}${GREEN}🎉 Battle complete! May the best config win! 🎉${RESET}"
